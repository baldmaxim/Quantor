...
| Часть                                                    | Статус                                                                                   |
| -------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| модели закреплены: ревизия и SHA-256 весов до скачивания | **PASS** — `fetch` 2B и 4B: `verified: true`                                             |
| матрица лицензий: Qwen, Unsloth, стек, адаптер           | готово ([матрица](model-license-matrix.md))                                              |
| окружение: закреплённые зависимости                      | **PASS** — wheel Unsloth совпали с матрицей; снимок `requirements-qwen-unsloth.lock.txt` |
| проверка GPU и BF16                                      | **PASS** — `probe`: RTX 5050, sm_120, BF16 работает, телеметрия Unsloth отключаема       |
| загрузка Unsloth + Qwen3-VL и ответ на синтетике         | **PASS** — 5 из 5 дымовых прогонов `OK` (§ 8)                                            |
| адаптер SFT из сборки промта 08                          | **PASS** — анти-утечка: все три проверки `true` (§ 8)                                    |
| строгий парсер ответа, синтетические тесты               | **PASS** — `pnpm test:vision` у владельца: 83 passed, ruff и mypy чисты                  |

Все строки подтверждены отчётом владельца 2026-09-15 с RTX 5050. Модель не обучалась.
...
## 8. Результаты на RTX 5050 (2026-09-15)

**Окружение.** Wheel `unsloth-2026.9.4` `7892ac14…5264` и `unsloth_zoo-2026.9.3` `d846a0cd…350c` совпали
с матрицей. Снимок — [`vision/requirements-qwen-unsloth.lock.txt`](../../vision/requirements-qwen-unsloth.lock.txt),
85 пакетов; версии прямых зависимостей совпали с закреплёнными (`version_mismatches: {}`); из
транзитивных — `triton-windows 3.8.0.post28`, `torchao 0.18.0`, `diffusers 0.40.0`,
`cut-cross-entropy 25.1.1`. Лицензионный гейт по снимку: нарушений нет.

**`probe`:** `OK`; RTX 5050, capability 12.0, `sm_120` в сборке torch, BF16 поддержан и произведение
матриц в BF16 конечно; 7,96 ГиБ. `UNSLOTH_DISABLE_STATISTICS` найден в `unsloth/models/_utils.py` —
выключение статистики переменной подтверждено по исходнику.

**Дымовые прогоны** (синтетический «чертёж», `HF_HUB_OFFLINE=1`):

| #   | Модель / стек / точность   | Статус | Пик VRAM, ГиБ | Разбор ответа        |
| --- | -------------------------- | ------ | ------------: | -------------------- |
| 1   | 2B / transformers / bf16   | OK     |          4,40 | `prose_or_fence`     |
| 2   | 2B / Unsloth / bf16        | OK     |          4,25 | `prose_or_fence`     |
| 3   | 4B / Unsloth / 4 бита      | OK     |          3,16 | разобран, объектов 0 |
| 4   | 4B / transformers / 4 бита | OK     |          3,60 | разобран, объектов 0 |
| 5   | 4B / transformers / bf16   | OK     |          8,99 | разобран, объектов 0 |

- Unsloth + Qwen3-VL на Windows и RTX 50 **работает** — запасной путь без Unsloth не нужен, но тоже
  проверен (строки 1, 4, 5): адаптер промта 13 грузится без Unsloth.
- **4B в BF16 запустился**, но пик 8,99 ГиБ больше 7,96 ГиБ карты: драйвер Windows (WDDM) досылает
  память из системной — это работает медленно и для обучения не годится. Запись Р-4 «4B в BF16 не
  помещается» остаётся верной для обучения; для одиночного инференса 4B BF16 возможен, скорость —
  по `timings` записи.
- До обучения 2B отвечает прозой (ответ не проходит строгий парсер), 4B — корректным JSON, но без
  плит на синтетике. Это наблюдение, а не метрика: промт 13 меряет разбор на val.
- Unsloth пишет `unsloth_compiled_cache/` в рабочий каталог (`vision/`), и ruff находил там тысячи
  ложных нарушений. Каталог закрыт `.gitignore` и `extend-exclude` ruff.

**SFT-набор `qwen-sft-v1`:**

| Вид                         | train | val | test | Отброшено обрывков (train/val/test) |
| --------------------------- | ----: | --: | ---: | ----------------------------------: |
| `qwen_slab_localization_v1` |    77 | 150 |   56 |                               5/2/2 |
| `qwen_masonry_roi_v1`       |   179 |  24 |   24 |                                   — |

Анти-утечка: `labels_generate_train_answers_only`, `evaluation_tiles_grid_only`,
`evaluation_rows_have_no_answer` — все `true`. `qwen_slab_polygon_v0` — не строится (Р-1).

**Вывод для промта 13.** Стек готов. Главное ограничение — не память, а данные: 77 train-тайлов плиты
одного проекта. Это совпадает с выводом SU10 о недостаточной поддержке плит (Р-7) и делает загрузку
новых проектов PlanSwift (О-5) важнее выбора размера модели.
