...
def polygon_area_pdf_points2(points: list[NormalizedPoint], geometry: PageGeometryValue) -> float:
    ...
    Самопересечения здесь не проверяются: формула шнурков на них даёт разность площадей, а не
    отказ. Политика — в правиле подсчёта: недействительный контур величины не получает вовсе
    (ADR-0026, `services/geometry/validity.py`).
    """
    if len(points) < 3:
        raise DomainError(
            ErrorCode.VALIDATION_FAILED,
