"""Исполнение заданий на Stage 1.

Задание выполняется фоновой задачей внутри процесса API. Это осознанно простой механизм:
реальная нагрузка сейчас одна, и ставить ради неё очередь с отдельными воркерами значит
завести инфраструктуру, которую придётся сопровождать без отдачи (ADR-0005).

Что здесь важно и переживёт замену исполнителя:

- состояние задания живёт в базе, а не в памяти процесса, и переживает перезапуск;
- у задачи своя сессия: сессия HTTP-запроса закрывается раньше, чем работа заканчивается;
