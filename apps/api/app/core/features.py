...
Пилотная возможность — третья категория (ADR-0023): она сделана и проверена, но выключена
по умолчанию и включается администратором на конкретное пространство. Такой флаг закрывает
возможность не только в интерфейсе, но и на сервере: иначе «выключено» в админке неправда.
...
def _pilot(key: str, title: str, description: str, *, stage: str) -> FlagDefinition:
    """Пилотная возможность: готова, выключена по умолчанию, включается на пространство.

    Системное умолчание остаётся `False` намеренно: пилот — решение о конкретном арендаторе,
    а не о всей установке, и неизвестное пространство возможность не получает.
    """
    return FlagDefinition(
        key=key,
        title=title,
...
REGISTRY: Final[MappingProxyType[str, FlagDefinition]] = MappingProxyType(
    {
        ...
        for definition in (
            ...
            # Готов с закрытия Stage 2A (PASS 15 / FAIL 0, живая приёмка на эталонном листе).
            # Включается владельцем для пилотного пространства, а не для всех сразу.
            _pilot(
                "takeoff.manual",
                "Ручные измерения",
                "Калибровка масштаба, счёт, линии, ломаные и площади по чертежу. Проверено "
                "живой приёмкой Stage 2A; готово к пилоту и включается на пространство.",
                stage="Этап 2A · пилот",
            ),
